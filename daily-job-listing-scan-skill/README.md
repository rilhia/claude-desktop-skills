...
- **Claude's built-in browser signed in to LinkedIn**, if you want to scan LinkedIn. The skill uses the browser inside the Claude Desktop app, not your own web browser. Step 5 shows how to sign in.
...
2. **LinkedIn:** the skill uses the browser built into Claude Desktop, which is separate from Chrome, Safari or Edge. Being signed in to LinkedIn in your normal browser doesn't count. To sign in:
   1. In Claude Desktop, open the browser pane with Cmd+Shift+B on a Mac or Ctrl+Shift+B on Windows. You can also click the globe icon (the Browser button) in the side panel.
   2. Go to linkedin.com in that pane and sign in as usual.
   You only need to do this once. The built-in browser remembers your sign-in for later tasks, including scheduled runs. The skill never tries to sign in for you. If LinkedIn ever signs you out, the email will say so, and you can sign in again the same way.
...
| "LinkedIn needs you to sign in" | Open Claude's built-in browser (Cmd+Shift+B or Ctrl+Shift+B), go to linkedin.com and sign in again. |
